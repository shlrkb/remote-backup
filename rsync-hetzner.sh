...
mysql -h 127.0.0.1 -u $MYSQLUSER -p$MYSQLPW -N -e 'show databases' | grep -Ev "^(Database|mysql|performance_schema|information_schema)$" | while read dbname; do mysqldump -h 127.0.0.1 -u $MYSQLUSER -p$MYSQLPW --single-transaction --no-tablespaces --complete-insert --triggers --routines "$dbname" | gzip -c > $MYSQLDIR/"$dbname".sql.gz; done
